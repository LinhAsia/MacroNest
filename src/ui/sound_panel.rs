        ...
        ui.allocate_ui_with_layout(
            ...
            |ui| {
                ...
                scroll_area.show(ui, |ui| {
                    ...
                    let pan_left =
                        playback_hotkeys_active && ui.input(|input| input.key_down(egui::Key::A));
                    let pan_right =
                        playback_hotkeys_active && ui.input(|input| input.key_down(egui::Key::D));
                    ...
                    outcome.preview_at_playhead = playback_hotkeys_active
                        && !ui.ctx().wants_keyboard_input()
                        && ui.input(|input| input.key_pressed(egui::Key::Space));
                    outcome.preview_from_trim_start = playback_hotkeys_active
                        && !ui.ctx().wants_keyboard_input()
                        ...
                        && (end_response.clicked() || end_response.dragged())
                    {
                        ...
                        *preview_cursor_ms = clip.end_ms;
                        ...
                        && total_ms > 0
                    {
                        let pointer_in_timeline =
                            pointer_pos.filter(|pos| viewport_rect.contains(*pos));
                        let playhead_dragging = ui
                            .ctx()
                            .data(|data| data.get_temp::<bool>(playhead_drag_id))
                            .unwrap_or(false);
                        let primary_pressed = ui.input(|input| input.pointer.primary_pressed());
                        let primary_down = ui.input(|input| input.pointer.primary_down());
                        let should_update_playhead = pointer_in_timeline.is_some()
                            && ((primary_pressed && response.hovered())
                                || (playhead_dragging && primary_down)
                                || response.clicked()
                                || response.dragged());
    ...
    pub(crate) fn render_sound_panel(&mut self, ui: &mut egui::Ui) {
        ...
        ui.horizontal(|ui| {
                ...
                .clicked()
            {
                ...
                self.state.audio_settings.presets.push(new_preset);
    ...
    fn render_audio_media_editor(
        ...
        let mut claim_preview_cursor = previewing
            || preview_cursor
                .as_ref()
                .is_some_and(|(cursor_target, _)| *cursor_target == target);
        let playback_hotkeys_active = claim_preview_cursor;
        let previous_item_spacing = ui.spacing().item_spacing;
        ui.spacing_mut().item_spacing = vec2(6.0, 4.0);
                        ...
                        if trim_timeline_outcome.preview_from_trim_start {
                            ...
                            if previewing {
                                ...
                                match audio::start_preview_from_ms(clip.clone(), preview_start_ms) {
                                    Ok(()) => {
                                        previewing = true;
                                        outcome.status = Some("Previewing.".to_owned())
                                    }
                                    Err(error) => {
                                        outcome.status = Some(format!("Preview failed: {error}"))
                                    }
